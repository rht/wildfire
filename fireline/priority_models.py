...
def validate_scenario(s):
    ...
    for tags in [s.capabilities, *(a.capabilities for a in s.actions)]:
        if any(not isinstance(tag, str) or not tag for tag in tags):
            raise ValueError('capability tags must be nonempty strings')
    for a in s.actions:
        ...
        for b in a.benefits:
            ...
            if not isinstance(b.evidence, str) or (b.confirmed and not b.evidence.strip()):
                raise ValueError('confirmed benefit needs evidence or a labelled assumption')
...
def scenario_from_dict(data):
    ...
    scenario = StaticScenario(
        ...
        actions=tuple(Action(**{**a, 'benefits': tuple(Benefit(**b) for b in a['benefits']),
                                'requires': tuple(a.get('requires', [])),
                                'capabilities': tuple(a.get('capabilities', []))}) for a in data['actions']),
        travel={(leg['from'], leg['to']): Travel(leg['minutes'], leg.get('available_until_min'))
                for leg in data['travel']},
        start_id=data.get('start_id', 'START'), horizon_min=data['horizon_min'],
        capabilities=tuple(data.get('capabilities', [])), buffer_min=data.get('buffer_min', 0))
