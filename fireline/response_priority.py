...
# Inclusive windows tolerate arithmetic noise only (60 nanoseconds in minute units).
TIME_EPSILON_MIN = 1e-9


def after_deadline(time, deadline):
    return time - deadline > TIME_EPSILON_MIN
...
class Planner:
    def __init__(self, scenario):
        ...
        self.s = scenario
        self.locations = {
            a.asset_id: a for a in sorted(scenario.locations, key=lambda a: a.asset_id)
        }
        ...
        for action in self.actions:
            ...
            for effect in sorted(action.benefits, key=lambda b: b.asset_id):
                ...
                missing = [
                    key
                    for key in ("people", "assisted", "value", "deadline_min")
                    ...
                    self.review.append(
                        {
                            ...
                            "reason": "unconfirmed effect"
                            if not effect.confirmed
                            else "unknown " + ", ".join(missing),
                        }
                    )
                else:
    ...
    def initial(self):
        return State(
            ...
            0,
            frozenset(),
            dict.fromkeys(self.locations, 0.0),
            (0.0, 0.0, 0.0),
            [],
        )
    ...
    def extend(self, state, action):
        ...
        leg = (
            Travel(0)
            if state.site == action.site_id
            else self.s.travel.get((state.site, action.site_id))
        )
        if leg is None:
            return None
        arrive = state.time + leg.minutes
        if leg.available_until_min is not None and after_deadline(
            arrive + self.s.buffer_min, leg.available_until_min
        ):
            return None
        finish = arrive + action.duration_min
        site = self.locations[action.site_id]
        if after_deadline(
            finish + self.s.buffer_min,
            min(self.s.horizon_min, action.deadline_min, site.deadline_min),
        ):
            return None
        ...
        for effect in self.effects[action.action_id]:
            target = self.locations[effect.asset_id]
            if after_deadline(finish + self.s.buffer_min, target.deadline_min):
                continue
            ...
            if gain:
                ...
                for i, amount in enumerate(
                    (target.assisted, target.people, target.value)
                ):
                    timing[i] += gain * amount * finish
        step = {
            "action_id": action.action_id,
            "site_id": action.site_id,
            "depart_min": state.time,
            "travel_min": leg.minutes,
            "start_min": arrive,
            "finish_min": finish,
            "prerequisites": list(action.requires),
            "coverage_gained": gained,
            "benefit_evidence": {
                e.asset_id: e.evidence
                for e in self.effects[action.action_id]
                if e.asset_id in gained
            },
        }
        return State(
            action.site_id,
            finish,
            state.done | {action.action_id},
            coverage,
            tuple(timing),
            state.steps + [step],
        )
    ...
    def totals(self, state):
        return tuple(
            sum(
                state.coverage[key] * getattr(a, field)
                for key, a in self.locations.items()
                if getattr(a, field) is not None
            )
            for field in ("assisted", "people", "value")
        )
    ...
    def better(self, candidate, current):
        ...
        if ck != bk:
            return ck > bk
        return tuple(step["action_id"] for step in candidate.steps) < tuple(
            step["action_id"] for step in current.steps
        )

    def result(self, state, optimal, visited):
        totals = self.totals(state)
        return {
            "scenario_id": self.s.scenario_id,
            "optimal": optimal,
            "method": "exact enumeration"
            if optimal
            else "direct-value greedy baseline",
            "states_evaluated": visited,
            "steps": state.steps,
            "finish_min": state.time,
            "objective": dict(
                zip(("assisted_units", "people_units", "value_units"), totals)
            ),
            "coverage": state.coverage,
            "unserved": {
                key: "not fully covered within the chosen feasible plan; review resources, deadlines and assumptions"
                for key, fraction in state.coverage.items()
                if fraction < 1
            },
            "review": self.review,
            "blocked_actions": self.blocked,
            "assumptions": [
                "one crew; no return-to-base requirement",
...
def plan_response(scenario):
    ...
    result["first_action_alternatives"] = [
        {
            "first_action": first,
            "sequence": [step["action_id"] for step in state.steps],
            "objective": dict(
                zip(
                    ("assisted_units", "people_units", "value_units"),
                    planner.totals(state),
                )
            ),
            "finish_min": state.time,
        }
        for first, state in sorted(first_best.items())
    ]
    return result
...
def greedy_response(scenario):
    ...
    while True:
        ...
        _, state = min(
            feasible,
            key=lambda pair: (
                -(planner.locations[pair[0].site_id].value or 0),
                pair[0].action_id,
            ),
        )
        visited += 1
