...
def rank_contacts(locations, policy=ContactPolicy()):
    ...
    weights = [policy.proximity_weight, policy.people_weight, policy.value_weight, policy.assisted_weight]
    for w in weights:
        number(w, 'weight')
    ...
    ranked, review = [], []
    for a in locations:
        missing = [field for field in ('distance_m','people','assisted','value') if getattr(a,field) is None]
        row = {'asset_id': a.asset_id, 'name': a.name, 'policy': policy.version,
               'score': None, 'rank': None, 'components': {}, 'review_reasons': missing}
        if missing:
            review.append(row)
            continue
        components = {
            'proximity': (1 / (1 + a.distance_m / policy.distance_scale_m), policy.proximity_weight),
            'people': (min(a.people / policy.people_scale, 1), policy.people_weight),
            'value': (min(a.value / policy.value_scale, 1), policy.value_weight),
            'assisted': (min(a.assisted / policy.people_scale, 1), policy.assisted_weight),
        }
        row['components'] = {key: {'normalised': value, 'weight': weight, 'contribution': value * weight}
