...
def coordinate_approved_evacuation(scenario, assessments, store, *, as_of,
    ...
    for row in result['locations']:
        ...
        row['mode'] = 'undetermined' if row['human_followup'] else (
...
def build_approved_recommendation(store, asset_id, *, as_of, snapshot_id, route_guidance):
    ...
    if (plan['snapshot_id'] != snapshot_id or not rows or not isinstance(route_guidance, dict)
