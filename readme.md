...
## Norma decision review: one accepted fix and one historical defense

This is a review proposal, not an exception submission. All six eligible feature branches were already pushed with zero reported findings on their final checked files; JavaScript coverage remains reduced because one server rule could not run. [PR #10](https://github.com/rht/wildfire/pull/10) records those results. No current changed-file finding remains from that work to preserve. The candidate below is therefore a **historical demonstration**, reproduced from the unchanged frozen audit source. Main keeps the merged remediation; no flagged code is reintroduced and SLNG remains deferred.

**Accepted change:** retain hardened JUnit XML parsing in [merged main, `scripts/build_priority_report.py:436`](https://github.com/rht/wildfire/blob/587f5bd53eb41bd04d693e49a60940b6a62530f1/scripts/build_priority_report.py#L436), with `defusedxml` and explicit DTD rejection. The original parser probe demonstrated internal entity expansion, not external-file disclosure. Regression tests cover rejecting DTD/entity declarations, accepting ordinary JUnit/XML declarations, and retaining failed-test behavior. This is a concrete input-hardening improvement already merged in [PR #3](https://github.com/rht/wildfire/pull/3).

**Proposed defense candidate:** branch `codex/qualityclouds-audit`, exact frozen SHA `92ae882545ae6b45fb6e2356e1f8aa17da599312`, [file `fireline/response_priority.py`, line 212, column 9](https://github.com/rht/wildfire/blob/92ae882545ae6b45fb6e2356e1f8aa17da599312/fireline/response_priority.py#L212), statement `nonlocal best, visited`, rule `py-arch-global-keyword` (LOW, architecture). The original report audited code SHA `5141a8b2d23f872e13c6c5831da9979b30815baf`; the audit commit adds its report. A fresh MCP Livecheck of the exact frozen file bytes returned this one finding, unreduced coverage, and no cap. File SHA-256: `2f65124e7303db5ed72ff10ee7b473c11b4a9ec914c502280bd29c491261bc20`. The [separate evidence JSON](reports/norma-historical-defense-2026-09-19.json) preserves the raw verdict and verification output.

Intended behavior: `plan_response` performs deterministic, synchronous depth-first enumeration of feasible action sequences for one crew, bounded by input validation to at most eight actions. Each call constructs its own `Planner`, `best`, `visited`, `first_best` and nested `search` function. `best` tracks the best feasible state across that call's traversal and `visited` counts every visited state. Sharing these two closure cells among recursive frames is intentional. Neither cell is module-level state, and the closure is invoked locally and does not escape. Separate calls create separate cells.

Why a bounded defense fits: the rule accurately matches `nonlocal`, but its description warns about mutation of module-level globals and hidden dependencies between independent code. That specific failure mode is absent here. A reasonable historical engineering decision would retain these two local accumulators because their lifetime and ownership match one complete search, keeping recursive calls focused on the state being explored. This is a scope-specific rationale, not a claim that all `nonlocal` use is harmless or that the rule is useless.

Tradeoffs: the closure has mutable implicit dependencies, so a reader must inspect its enclosing function; testing the inner search independently is less convenient. Explicit parameter/return state, as now merged on main, is also a sound design and makes data flow more visible. No performance advantage has been measured or claimed. Enumeration cost and the eight-action product limit are separate concerns; the closure does not solve combinatorial scaling. The historical `first_best` dictionary is also shared within the call even though it does not require `nonlocal`, so the safety argument rests on lifetime and ownership rather than the absence of a keyword.

Evidence: 35 tests from the exact historical planner test file pass, including independent permutation oracles across 12 simple and 30 constrained four-action scenarios, tie ordering, fractional deadlines, prerequisites, capabilities and input validation. An additional probe loaded the hash-verified historical module from an isolated archive: 80 sequential calls interleaved across different scenarios returned identical complete results; one nested invocation returned the correct independent result while the outer traversal remained unchanged; 80 independent calls through four worker threads also matched. Feasible zero-, one-, two- and four-action examples visited exactly 1, 2, 5 and 65 states. Inputs remained unchanged and neither `best` nor `visited` existed in the module namespace. These finite tests support call isolation; they do not prove all concurrency properties or validate real-world response outcomes.

Invalidate/revisit the defense if recursive branches run concurrently within one invocation, the closure escapes or is reused across requests, accumulators move into a shared object/module, callbacks gain access to mutate traversal state, callers mutate scenario inputs during execution, or resumable/asynchronous search changes the ownership/lifetime model. Expanding the action bound also requires a new scalability review. Under any of these conditions, choose explicit owned state and appropriate synchronization or redesign; do not silently extend this historical rationale.

Alternatives evaluated: the historical migration SQL at `fireline/tasks.py:189` interpolates only a locally fixed tuple of column/type literals, which supports a narrower data-flow defense than a generic SQL-injection warning. Nevertheless, fixed DDL is straightforward and already merged. The `_next_id` SQL at line 201 has only literal callers in this revision, but accepts arbitrary strings without an internal whitelist, making that defense depend on future caller discipline. The per-invocation DFS is the stronger demonstration. None of these historical observations justify reverting the merged SQL or planner improvements.

Reproduction leaves the frozen worktree untouched: export `git archive 92ae882545ae6b45fb6e2356e1f8aa17da599312` into an isolated directory, run `python -m pytest -q -p no:cacheprovider tests/test_static_priorities.py` there with the project's development dependencies, then run the probe source stored as `verification.isolation_probe_source` in the evidence JSON from a file whose sibling `historical/` directory contains that archive. The probe checks the exact module hash before invoking it. Recheck the historical file with MCP `live_check`, preserving its original filename and full content; do not scan a rewritten snippet. Independent code review approved this historical demonstration and independently reran the 35 tests and isolation probe. The defense awaits user selection before any formal exception submission; no exception, suppression or accepted-risk registration has been submitted.
