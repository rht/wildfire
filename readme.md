...
## Norma decision review: branch-specific defense before remediation

The primary proposal is the evacuation-readiness branch's strict integer capacity check, using its actual pre-remediation revision and phase-two scan evidence. All six eligible branches have zero reported findings **after remediation** (with reduced JavaScript coverage); that does not mean their original findings lacked defensible design choices. The implementation discussed below has already been refactored. This review does not revert it, change production code, or submit an exception. [PR #10](https://github.com/rht/wildfire/pull/10) retains the original phase-two ledger and raw checks; [the separate branch-defense evidence](reports/norma-branch-defense-2026-09-19.json) links exact revisions, file hashes, recorded attempts, a fresh recheck and comparison tests.

**Primary candidate:** branch `codex/evacuation-readiness`, original SHA `f86798cd9269c59b502c06fe7c78a590c7097ec9`, [`fireline/evacuation_readiness.py:103`](https://github.com/rht/wildfire/blob/f86798cd9269c59b502c06fe7c78a590c7097ec9/fireline/evacuation_readiness.py#L103), column 12, rule `py-arch-type-eq` (HIGH, architecture). The flagged statement is `if type(centre.remaining_places) is not int or centre.remaining_places < 0:`. Zero-based raw attempt 13 records this actual finding. Its SHA-256 exactly matches the original file: `e69a2aed4396affbc6b9c31488cf0bd32464ae9e4b6571b9c3ed88fa3a2b23cf`. A fresh full-file MCP Livecheck reproduced the same single finding with unreduced coverage and no cap.

**Proposed conscious decision:** require a nonnegative built-in integer for a receiving centre's remaining place count. Reject booleans, floats (including `10.0`), strings, null, integer subclasses and `IntEnum` values at this boundary instead of coercing them. This is a defensible strict primitive-input contract supported by the original implementation; it is not a claim to know undocumented author intent. The rule's general preference for accepting valid subtypes does not decide which types this boundary promises to accept. In Python, `isinstance(True, int)` is true, and a JSON `true` becomes that boolean. A plain replacement with `isinstance(value, int)` plus a nonnegative check would therefore accept a boolean as a count. That counterexample supports rejecting bool; it does not by itself require rejecting every other integer subtype.

**Tradeoff and current alternative:** exact-type checking deliberately rejects otherwise usable integer subclasses and enum counts, reducing extensibility and requiring explicit normalization by callers that use such types. This is a narrow domain-boundary choice, not a general recommendation against polymorphism. The current published revision `d75d4b769940e0ca6b0a01949f5c7bf84daec44e` explicitly rejects bool and then checks `isinstance(..., int)`; it preserves boolean rejection while accepting nonnegative integer subclasses. Both are coherent policies. The current fix is not wrong, and this proposal does not automatically restore the stricter one. A decision to restore exact-type validation would require separately selecting that input contract.

**Evidence:** isolated archives of the exact original and current revisions pass their complete readiness suites: 36 original tests and 41 current tests. A hash-verified public-API probe ran 11 capacity cases against each revision: zero, positive and negative integers, true, false, integral/fractional floats, string, null, an ordinary integer subclass and an `IntEnum`. Both reject bool and all malformed primitive cases; only the subtype cases differ. For accepted capacity 10, the fixture allocates eight places and leaves two; zero allocates none. A JSON boolean probe confirms that a bare `isinstance`/nonnegative alternative would accept `true`. Initial archive CLI test runs picked up another worktree's editable package; setting `PYTHONPATH=.` to each tested archive resolved those import failures without code changes. The evidence records both the setup failure and successful runs. These tests establish type and allocation semantics, not real-world evacuation safety.

**Invalidation conditions:** revisit this defense if the public contract must accept integer subclasses, enum/numerical-library counts or fractional capacity, or if an upstream schema takes ownership of normalization and strict runtime checking no longer serves this boundary. Rejecting bool must remain explicit in any broader integer policy. Do not replace rejection with silent coercion of malformed values. The decision should be documented as a primitive capacity contract and covered by tests before being selected for future implementation.

Other genuine branch-specific candidates were also present:

- `claude/ca-calibration`, original SHA `856487189fc122226a33fac11a7623e36b9bb918`, `fireline/calibrate.py:82,132`, `fa-sec-path-traversal`, zero-based attempt 6. The scanned bytes match that revision. These functions are local `lru_cache` loaders for caller-selected fixture paths, not HTTP request handlers. Allowing the local caller to choose readable files can be intentional; introducing an arbitrary root restriction would change that API. This rationale stops applying if an untrusted remote caller controls paths or the process gains authority the caller should not have. Path normalization is not an authorization boundary. The current refactor normalizes cache paths and uses UTF-8; it remains in place.
- `claude/workflow-setup`, original SHA `367dddca72895bae50b25f0cfa33ab39ac9d1f96`, `js-scl-foreach-async`, zero-based attempt 14. Its extracted inline JavaScript hash matches the original revision exactly. Virtual lines 114, 127 and 189 correspond to original HTML lines 318, 331 and 393. The recorded check used a later SRI-only HTML layout, so its start-tag line 207 differs from the original start-tag line 205; the script bytes are identical. These callbacks perform synchronous scoring/DOM/marker work, contain no async callback, and do not return promises for the loop to await. They are false positives for the rule's stated async-callback concern. That defense would fail if asynchronous operations were introduced or promises returned without awaiting them. It does not defend the separate unsafe HTML assignments in the same scan, which were correctly fixed. The current loop and DOM refactors remain in place; the recorded JavaScript scan has reduced coverage.

**Accepted change:** retain the merged JUnit XML hardening in [`scripts/build_priority_report.py:436`](https://github.com/rht/wildfire/blob/587f5bd53eb41bd04d693e49a60940b6a62530f1/scripts/build_priority_report.py#L436). `defusedxml` with explicit DTD rejection addresses the demonstrated internal-entity behavior; external-file disclosure was not demonstrated. The tests preserve ordinary JUnit/XML declaration support and failed-test behavior. [PR #3](https://github.com/rht/wildfire/pull/3) remains merged.

The older main-audit DFS `nonlocal best, visited` example is retained only as a secondary historical appendix in [its evidence JSON](reports/norma-historical-defense-2026-09-19.json). Its real finding and isolation tests remain valid, but it is no longer the preferred candidate when the actual branch scans already supply defensible choices.

Reproduction: export the original and current readiness SHAs with `git archive` into isolated directories, then run `PYTHONPATH=. python -m pytest -q -p no:cacheprovider tests/test_evacuation_readiness.py` in each with the project's development dependencies. Save the JSON's `probe_source` as a Python file and run it with arguments `ARCHIVE_PATH EXPECTED_FILE_SHA256 original|current`; it checks the exact loaded module hash. No original audit or feature-worktree files are modified. SLNG and its untracked lockfile remain untouched. This is a defense proposal for review; no formal exception, suppression or accepted-risk registration has been submitted.
